...
TexasHoldEm::TexasHoldEm() : dealer(0), common_bet(0), highest_bet(0), oneleft(false), phase(0) { //static definition
	for (int i = 0; i < 13; ++i) {
		for (int j = 0; j < 4; ++j) {
...
bool TexasHoldEm::poker_rankTexas(shared_ptr<Player> p1, shared_ptr<Player> p2)
{
...
//not implemennted
bool TexasHoldEm::call(Player& a) {
	return 0;
}
//not implemennted
bool TexasHoldEm::raise(Player& a, int raise) {
	return 0;
}
//not implemennted
int TexasHoldEm::before_turn(Player & a) {
	return 0;
}
...
int TexasHoldEm::turn(Player & a)
{
	...
	while (i != pos) {
		...
		std::cout << " Hand: " << vpPlayer[i]->playersCards.asStringTexas() << flush;// only prints those visible
...
int TexasHoldEm::before_round()
{
	...
	for (int i = 0; i < 2; ++i) { // deal first 2 cards to all players
		do {
...
int TexasHoldEm::round()
{
	...
	for (int i = 0; i <= 3;++i) {
		...
		*/
		//go to all players and let them bet and deal cards accordingly
		goodturn = false;
		while (!goodturn && oneleft == false) {

			//go to all players and they bet
			do {
				player_size = vpPlayer.size();
		...
		if (oneleft == false) {
			//after phase 0, deal 3 cards
			if (phase == 0) {
				pos = 3;
				do {
					mainDeck.callShuffle();
					sharedcards.hand.push_back(mainDeck.deck[mainDeck.deck.size() - 1]);
					mainDeck.deck.pop_back();
					pos--;
				} while (pos >0);

				for (size_t k = 0; k < vpPlayer.size();++k) {
					vpPlayer[k]->playersCards.hand.push_back(sharedcards[0]);
					vpPlayer[k]->playersCards.hand.push_back(sharedcards[1]);
					vpPlayer[k]->playersCards.hand.push_back(sharedcards[2]);
					
				}
			}
			else if (phase ==1 ) { // after phase 1 deal 1 card
				mainDeck.callShuffle();
				sharedcards.hand.push_back(mainDeck.deck[mainDeck.deck.size() - 1]);
				mainDeck.deck.pop_back();
				for (size_t k = 0; k < vpPlayer.size();++k) {
					vpPlayer[k]->playersCards.hand.push_back(sharedcards[3]);
				...
				mainDeck.deck.pop_back();
				for (size_t k = 0; k < vpPlayer.size();++k) {
					vpPlayer[k]->playersCards.hand.push_back(sharedcards[4]);
				}
			}
			else {
				phase = 0;
...
int TexasHoldEm::after_round()
{
	if (oneleft == false) {
		...
		for (int i = 0; i < 7; ++i) {
			mainDeck.add_card(vpPlayer[0]->get_hand()[i]);
		}

		for (size_t count = 1; count < vpPlayer.size();++count) {
			mainDeck.add_card(vpPlayer[count]->get_hand()[0]);
			mainDeck.add_card(vpPlayer[count]->get_hand()[1]);
		}
		...
		std::sort(vpPlayer.begin(), vpPlayer.end(), TexasHoldEm:: poker_rankTexas);
		...
		for (size_t i = 1; i < vpPlayer.size(); ++i) {
			...
			std::sort(t1.begin(), t1.end(), TexasHoldEm::poker_rankTexas);
			t2.push_back(vpPlayer[i]);
			t2.push_back(vpPlayer[i - 1]);
			std::sort(t2.begin(), t2.end(), TexasHoldEm::poker_rankTexas);
